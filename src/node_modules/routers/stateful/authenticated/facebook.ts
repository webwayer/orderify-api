...
export function facebookLoginRouterFactory(router: Router, config: FACEBOOK_CONFIG & { BASE_URL: string }, User: UserType, Album: AlbumType, Photo: PhotoType) {
    ...
    router.get('/login/facebook/callback', async (req, res) => {
        ...
        if (!error && state === req.session.facebook_login_csrf_token) {
            ...
            if (access_token) {
                ...
                await uploadPhotoLibraryFromFacebook(Album, Photo, user.id, access_token)
    ...
    async function uploadPhotoLibraryFromFacebook(Album: AlbumType, Photo: PhotoType, userId: number, access_token: string) {
