...
export function photoLibraryOnFacebookFactory(Album: AlbumType, Photo: PhotoType, FacebookMetadata: FacebookMetadataType, images: ReturnType<typeof imagesFactory>, facebookGraph: ReturnType<typeof facebookGraphFactory>) {
    async function download(userId: number, access_token: string) {
        ...
        for (const album of library) {
            ...
            Promise.all(ourPhotos.map(ourPhoto => images.uploadFromUrl(ourPhoto.id, ourPhoto.link)))
        }
    }
